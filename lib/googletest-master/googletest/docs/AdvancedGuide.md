...
template <typename V> class Foo {
 public:
  void Bar() { ::testing::StaticAssertTypeEq<int, V>(); }
};
```
the code:
```
...
class FooTest : public ::testing::Test {
 protected:
  ...
  static V* shared_resource_;
};

V* FooTest::shared_resource_ = NULL;
...
`::testing::WithParamInterface<V>` (the latter is a pure interface),
where `V` is the type of your parameter values.  For convenience, you
can just derive the fixture class from `::testing::TestWithParam<V>`,
which itself is derived from both `::testing::Test` and
`::testing::WithParamInterface<V>`. `V` can be any copyable type. If
