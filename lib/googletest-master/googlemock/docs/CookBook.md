...
`SafeMatcherCast<V>(m)` function. It casts a matcher `m` to type
`Matcher<V>`. To ensure safety, Google Mock checks that (let `U` be the
type `m` accepts):

  1. Type `V` can be implicitly cast to type `U`;
  1. When both `V` and `U` are built-in arithmetic types (`bool`, integers, and floating-point numbers), the conversion from `V` to `U` is not lossy (in other words, any value representable by `V` can also be represented by `U`); and
  1. When `U` is a reference, `V` must also be a reference (as the underlying matcher may be interested in the address of the `U` value).
...
If you find `SafeMatcherCast<V>(m)` too limiting, you can use a similar
function `MatcherCast<V>(m)`. The difference is that `MatcherCast` works
as long as you can `static_cast` type `V` to type `U`.
...
Google Mock, you can override it using `DefaultValue<V>::Set()`
...
one object to another, but cannot be copied. `std::unique_ptr<V>` is
...
Also, remember that `DefaultValue<V>` is a global resource that
...
A matcher of argument type `V` implements
`::testing::MatcherInterface<V>` and does two things: it tests whether a
value of type `V` matches the matcher, and can describe what kind of
...
class MatchResultListener {
 public:
  ...
  // is NULL.
  template <typename V>
  MatchResultListener& operator<<(const V& x);
...
class MatcherInterface {
 public:
  ...
  // result to 'listener'.
  virtual bool MatchAndExplain(V x, MatchResultListener* listener) const = 0;
...
Example:
```
// DuplicateArg<k, V>(output) converts the k-th argument of the mock
// function to type V and copies it to *output.
ACTION_TEMPLATE(DuplicateArg,
                // Note the comma between int and k:
                HAS_2_TEMPLATE_PARAMS(int, k, typename, V),
                AND_1_VALUE_PARAMS(output)) {
  *output = V(::testing::get<k>(args));
}
```
